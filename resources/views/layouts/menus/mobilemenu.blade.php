<div class="container d-flex justify-content-end">
    <div class="header-wrap header-action">
        <div class="header-action-icon d-block d-xl-none">
            <a class="mini-cart-icon" href="{{ url('/user/viewcart') }}">
                ...
                @if (!empty($cartItemCount))
                    <span class="pro-count blue" style="bottom: -5px; left: 18px">{{ $cartItemCount }}</span>
                @endif
            </a>
        </div>
        <div class="header-action-icon-2 d-block d-xl-none ms-4 mb-1">
            ...
            <div class="mobile-menu-wrap mobile-header-border">
                ...
                <nav>
                    <ul class="mobile-menu font-heading">
                        <li class="menu-item">
                            <a href="/products"
                                style="color: {{ request()->is('products*') || request()->is('viewproduct*') ? '#e10000' : '' }}">{{ __('menu.products') }}</a>&nbsp;&nbsp;
                        </li>
                        <li class="menu-item">
                            <a href="/rootcategories"
                                style="color: {{ request()->is('rootcategories*') || request()->is('innercategories*') ? '#e10000' : '' }}">{{ __('menu.categories') }}</a>&nbsp;&nbsp;
                        </li>
                        <li class="menu-item">
                            <a href="/promotions"
                                style="color: {{ request()->is('promotions*') || request()->is('promotion*') ? '#e10000' : '' }}">{{ __('menu.promotions') }}</a>&nbsp;&nbsp;
                        </li>
                        <li class="menu-item">
                            <a href="/user/discountCoupons"
                                style="color: {{ request()->is('discountCoupons') ? '#e10000' : '' }}">{{ __('menu.discountCoupons') }}</a>&nbsp;&nbsp;
                        </li>
                        <li class="menu-item">
                            <a href="/user/messenger"
                                style="color: {{ request()->is('messenger*') ? '#e10000' : '' }}">{{ __('menu.messenger') }}</a>&nbsp;&nbsp;
                        </li>
                        <li class="menu-item">
                            <a href="/eu_projects"
                                style="color: {{ request()->is('eu_projects') ? '#e10000' : '' }}">{{ __('menu.euProjects') }}</a>&nbsp;&nbsp;
                        </li>
                        <div style="height: 30px"></div>
                        @auth
                            {{-- <li class="menu-item">
                                <a href="{{ url('/user/viewcart') }}"
                                    style="color: {{ request()->is('user/viewcart*') ? '#e10000' : '' }}">
                                    <img alt="cart-icon" src="{{ asset('/images/theme/icons/icon-cart.svg') }}"
                                        width="15" class="me-1" />
                                    @if (!empty($cartItemCount))
                        ...
                        @else
                            @if (Route::has('login'))
                                <li class="menu-item">
                                    <a href="{{ route('login') }}"
                                        style="color: {{ request()->is('login*') ? '#e10000' : '' }}">
