       ...
       <tr>
          ...
          <td class="tp-cart-img">
             <a href="{{ route('viewproduct', $item['product']->id) }}"> 
                 <img alt="{{ $item['product']->name }}" class="product-thumbnail-image" 
                     src="@if ($item['product']->image) {{ $item['product']->image }} @else {{ asset('template/img/store/pro-1.jpg') }} @endif">
             </a>
         </td>
          <!-- title -->
          ...
          <td class="tp-cart-action">
             {!! Form::open(['route' => ['userCartItemDestroy', $item->id], 'method' => 'delete']) !!}
                 <button type="submit "class="btn btn-primary" onclick="return confirm('{{ __('messages.confirmDeleteProduct') }}')">
