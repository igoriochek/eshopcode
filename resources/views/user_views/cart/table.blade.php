            ...
            <tr>
                ...
                <td class="quantity">
                    <div class="input-counter">
                        <input type="text" style="border: none" title="Qty" value="{{ $item->count }}"
                            name="quantity" min="1" max="5" minlength="1" maxlength="5" readonly>
                    </div>
                </td>
                <td class="total">€{{ $item->price_current * $item->count }}</td>
                <td class="remove">
                    {!! Form::open(['route' => ['userCartItemDestroy', $item->id], 'method' => 'delete']) !!}
                    <button type="submit" class="btn" title="{{ __('names.removeProduct') }}"
