@forelse($returns as $return)
    <div class="d-flex gap-2 px-4 text-dark fs-6" style="border: 1px solid lightgray; @if ($loop->last) border-radius: 0 0 8px 8px; @endif">
        ...
        <div class="d-flex align-items-center py-3" style="width: 15%">{{ __("status." . $return->status->name) }}</div>
        <div class="d-flex align-items-center py-3" style="width: 35%">{{ $return->description }}</div>
        <div class="d-flex align-items-center py-3" style="width: 15%">€{{ number_format($returnItemSum, 2) }}</div>
