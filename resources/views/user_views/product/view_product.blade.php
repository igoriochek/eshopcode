                        ...
                        <div class="col-md-6 col-sm-12 col-xs-12 mb-md-0 mb-sm-5">
                            <div class="detail-gallery">
                                <div class="product-image-slider slick-initialized slick-slider"
                                    ...
                                    @else
                                        <img src="{{ asset('images/noimage.jpeg') }}" alt="no-product-image"
                                            class="w-100" />
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="detail-info pr-30 pl-30">
                                    ...
                                    <div class="product-rate-cover text-end">
                                        <div class="d-inline-block">
                                            <div class="d-flex" style="width: 90%">
                                                @for ($i = 1; $i <= 5; $i++)
                                                    <i
                                                        class="text-warning fs-6 @if ($average >= $i) fa-solid fa-star
                                                        @elseif ($average >= $i - 0.5) fa-solid fa-star-half-stroke
                                                        @else fa-regular fa-star @endif"></i>
                                                @endfor
                                            </div>
                                        </div>
                                        <span class="ps-2 fs-6">({{ $rateCount . ' ' . __('names.reviews') }})</span>
                                    </div>
                                </div>
                                        ...
                                        <div class="d-flex align-items-center gap-2">
                                            @foreach ($product->sizesPrices as $sizePrice)
                                                <div class="product-size-button">
                                                    <input type="radio" id="{{ $sizePrice->size->name }}" name="size"
                                    ...
                                    <div class="d-block mt-20">
                                        ...
                                        <div class="d-flex align-items-center gap-2">
                                            @foreach ($productMeats as $productMeat)
                                                <div class="product-size-button">
                                                    <input type="radio" id="{{ $productMeat->name }}" name="meat"
                                                        value="{{ $productMeat->id }}" />
                                    ...
                                    <div class="d-block mt-20">
                                        ...
                                        <div class="d-flex align-items-center flex-wrap gap-2">
                                            @foreach ($productSauces as $productSauce)
                                                <div class="product-size-button">
                                                    <input type="radio" id="{{ $productSauce->name }}" name="sauce"
                                                        value="{{ $productSauce->id }}" />
                                                    <label class="d-flex align-items-center justify-content-center"
                                                        for="{{ $productSauce->name }}">
                                                        <div
                                                            style="background: {{ $productSauce->color }}; width: 21px; height: 21px; border-radius: 13px">
                                                        </div>
                                ...
                                @if ($product->hasPaidAccessories)
                                    <div class="d-block mt-20">
                                        ...
                                        <div
                                            class="d-flex flex-column flex-sm-row align-items-sm-center flex-wrap gap-4 mt-3">
                                            @foreach ($paidAccessories as $paidAccessory)
                                                <div class="d-flex align-items-center gap-2">
                                                    <input type="checkbox" id="{{ $paidAccessory->name }}"
                                                        name="paidAccessory" value="{{ $paidAccessory->id }}" />
                                                    <label
                                                        class="d-flex align-items-center justify-content-center pt-1 fw-500"
                                                        for="{{ $paidAccessory->name }}"
                                                        style="font-size: 15px; line-height: 2px; color: #666">
                                                        {{ $paidAccessory->name }}
                                    ...
                                    <div class="d-block mt-30">
                                        ...
                                        <div
                                            ...
                                            @foreach ($product->freeAccessories as $freeAccessory)
                                                <div class="d-flex align-items-center gap-2">
                                                    <input type="checkbox" id="{{ $freeAccessory->name }}"
                                                        name="freeAccessory" value="{{ $freeAccessory->id }}" checked />
                                                    <label
                                                        class="d-flex align-items-center justify-content-center pt-1 fw-500"
                                                        for="{{ $freeAccessory->name }}"
                                                        style="font-size: 15px; line-height: 2px; color: #666">
                                                        {{ $freeAccessory->name }}
                                                    </label>
                                                </div>
                                ...
                                @endif
                                <div class="detail-extralink mb-50 @if (
                                    $product->hasSizes ||
                                        $product->hasMeats ||
                                        $product->hasSauces ||
                                        $product->hasPaidAccessories ||
                                        $product->hasFreeAccessories) mt-40 @endif">
                                    {!! Form::open(['route' => ['addtocart'], 'method' => 'post', 'class' => 'd-flex align-items-center']) !!}
                                    <div class="d-flex me-4">
                                        {!! Form::number('count', '1', [
                                            'style' => 'width: 80px; height: 51px; padding-left: 20px; padding-right: 15px; border-radius: 5px',
                                            'min' => '1',
                                            'max' => '5',
                                            'minlength' => '1',
                                            'maxlength' => '5',
                                    ...
                                    @if ($product->hasSizes)
                                        <input type="hidden" name="size" id="size"
                                            value="{{ $defaultSize->size->id }}">
                                    @endif
                                    @if ($product->hasMeats)
                                        <input type="hidden" name="meat" id="meat"
                                            value="{{ $defaultMeat }}">
                                    @endif
                                    @if ($product->hasSauces)
                                        <input type="hidden" name="sauce" id="sauce"
                                            value="{{ $defaultSauce }}">
                                    @endif
                                    @if ($product->hasPaidAccessories)
                                    ...
                                    <div class="product-extra-link2">
                                        <button type="submit" class="button button-add-to-cart"
                                            style="padding-inline: 30px; font-size: .95rem">
