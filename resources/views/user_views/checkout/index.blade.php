        ...
        <div class="row">
            <div class="col text-center">
                <p>{{ __('names.wantToApply') }}
                    <a href="#" class="open-apply-coupon" data-bs-toggle="collapse"
                        data-bs-target=".coupon-form-wrapper" aria-expanded="true">
                        ...
                        <div class="d-flex align-items-center flex-column flex-md-row">
                            <select name="discount[]" class="form-control h-auto border-radius-0 line-height-1 py-3">
                                ...
                                @foreach ($discounts as $item)
                                    <option value="{{ $item->id }}">{{ $item->code }} - {{ $item->value }} EU
                                        {{ __('names.off') }}</option>
                                @endforeach
                            </select>
                            <button type="submit"
                                class="btn apply-coupon-button mt-4 mt-md-0 ms-0 ms-md-3">{{ __('buttons.applyCoupon') }}</button>
                        </div>
                        {!! Form::close() !!}
                    </div>
                </div>
                                    ...
                                    <tr class="total border-bottom">
                                        ...
                                        <td class="text-end">
                                            <strong class="text-color-dark">
                                                <span
                                                    class="amount text-dark text-5">€{{ number_format($cart->sum, 2) }}</span>
                                            </strong>
                                        </td>
                                    </tr>
                                    <tr class="company-purchase border-bottom">
                                        <td colspan="2">
                                            <div class="d-flex flex-column">
                                                ...
                                                @enderror
                                                <div class="row mb-3 mt-2" id="company_purchase_form">
...
@push('scripts')
    <script>
        const toggleCompanyPurchaseForm = () => {
            const companyPurchaseCheckbox = document.getElementById('company_purchase')
            const companyPurchaseForm = document.getElementById('company_purchase_form')

            companyPurchaseCheckbox.value = 0;
            companyPurchaseForm.classList.toggle('d-none')

            companyPurchaseCheckbox.addEventListener('click', () => {
                companyPurchaseCheckbox.checked ? companyPurchaseCheckbox.value = 1 : companyPurchaseCheckbox
                    .value = 0
                companyPurchaseForm.classList.toggle('d-none')
            })
        }

        toggleCompanyPurchaseForm()
    </script>
@endpush
